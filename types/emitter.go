...
func NewEmitter[T any](closer closer.Closer) Emitter[T] {
	return &emitter[T]{
...
func (emitter *emitter[T]) Emit(value T) {
	emitter.mutex.RLock()
	defer emitter.mutex.RUnlock()

	for _, subscriber := range emitter.subscribers {
		go func(subscriber Subscriber[T]) {
			ctx, cancelFunc := context.WithCancel(emitter.closer.GetContext())
			defer cancelFunc()
